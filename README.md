...
> **Помним!** Это шестнадцатеричная 1000, которая в десятичной системе будет 4096
...
#### 1. Команда сравнения `CMP`

`CMP dest, src` — это, по сути, вычитание (`dest - src`), но результат никуда не записывается. Вместо этого процессор меняет состояние специальных **флагов** в регистре `EFLAGS`.

* Если числа равны, устанавливается флаг нуля (**ZF** = 1).
* Если `dest < src`, устанавливается флаг переноса (**CF** = 1).

#### 2. Условные переходы (`Jcc`)

Эти команды смотрят на флаги и решают: прыгать на "метку" или идти дальше.

| Команда | Описание | Условие (после CMP) |
| --- | --- | --- |
| **`JE`** (Jump Equal) | Прыжок, если равно | `dest == src` |
| **`JNE`** (Jump Not Equal) | Прыжок, если не равно | `dest != src` |
| **`JG`** (Jump Greater) | Прыжок, если больше | `dest > src` |
| **`JL`** (Jump Less) | Прыжок, если меньше | `dest < src` |
| **`JMP`** | Безусловный переход | Прыгает всегда |

---

### 🛠️ Новые задания

Давай попробуем реализовать простую логику.

**Задание 4: "Максимум из двух"**
Допустим, в `EAX` лежит число `15`, а в `EBX` число `42`. Напиши фрагмент кода, который сравнивает их и помещает **большее** из них в регистр `ECX`.
*(Подсказка: используй `CMP`, условный переход и метки, например `is_greater:`)*.

**Задание 5: "Простой цикл"**
Используя `DEC`, `CMP` и переход, попробуй реализовать цикл, который выполняется 5 раз. Пусть в `ECX` изначально лежит `5`, и в каждой итерации ты вычитаешь из него `1`, пока он не станет равен `0`.
